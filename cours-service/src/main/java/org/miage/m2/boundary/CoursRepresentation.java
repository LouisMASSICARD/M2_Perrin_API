...
public class CoursRepresentation {
    ...
    private final CoursResource utilisateurRessource;
    private final UtilisateurValidator validator;

    public CoursRepresentation(CoursResource utilisateurResource, UtilisateurValidator utilisateurValidator) {
...
//            return ResponseEntity.ok().build();
    ...
    private CollectionModel<EntityModel<Utilisateur>> utilisateurToResource(Iterable<Utilisateur> utilisateurs) {
        Link selfLink = linkTo(methodOn(CoursRepresentation.class).getAllUtilisateurs()).withSelfRel();
    ...
    private EntityModel<Utilisateur> utilisateurToResource(Utilisateur utilisateur, Boolean collection) {
        var selfLink = linkTo(CoursRepresentation.class).slash(utilisateur.getId()).withSelfRel();
        if (Boolean.TRUE.equals(collection)) {
            Link collectionLink = linkTo(methodOn(CoursRepresentation.class).getAllUtilisateurs())
                    .withRel("collection");
